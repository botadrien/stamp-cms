// Petit client pour l'API GitLab (v4) — miroir de ForgejoApi (app/providers/api.js) et GitHubApi
// (app/providers/github-api.js), même surface de méthodes pour que app/app.js/app/site/site-builder.js appellent
// l'un ou l'autre sans distinction (voir app/providers/providers.js pour le choix du bon client selon le
...
//   composer blob/tree/commit/ref à la main, voir app/providers/github-api.js).
...
//   (app/app.js) puisse lire `repo.owner.login`/`repo.name` sans distinction de fournisseur.
// - saveFile() renvoie `{content: {sha}}` comme Forgejo/GitHub pour que app/app.js puisse
//   continuer à lire `result.content.sha` (voir app/app.js:666) — GitLab n'expose pas
...
class GitLabApi {
  ...
  // topic SITE_TOPIC (voir app/providers/api.js) — contrairement à Forgejo/GitHub, l'API de création de
  ...
  // exposent directement `name`/`owner.login`/`full_name` — app/app.js lit ces trois derniers
  // champs sans distinguer le fournisseur (voir app/app.js:290, `r.full_name`), d'où cette
  ...
  // en pratique contre une vraie instance GitLab (voir tests/, plan d'implémentation).
...
`;
  ...
  // Sha du commit HEAD d'une branche — vérification légère utilisée par app/site/repo-cache.js
  ...
  // docs/plan-lecture-content-batch.md et app/site/tar-utils.js) — remplace listTree + N×getBlob,
  ...
  async fetchRepoArchive(owner, repo, ref) {
    ...
    // (app/providers/api.js) : cette URL ne varie pas avec le commit, un cache HTTP navigateur non
  ...
  // ForgejoApi.publishFiles (app/providers/api.js) pour le pourquoi (branche `pages` entièrement
  ...
  // (voir app/app.js:666, qui lit `result.content.sha` sans distinguer le fournisseur).
  ...
  // vraie instance GitLab (voir tests/).
  isConflict(err) {
...
// contrainte UTF-8 -> base64 que Forgejo/GitHub (voir bytesToBase64 dans app/providers/api.js, réservé
