...
// editor-src/ssg-builder.js et app/ssg/renderer.jsx) : contenu Markdown récupéré sur la
// branche main, buildé en mémoire avec les gabarits Puck par défaut
// (app/ssg/default-templates.js), publié sur la branche pages. Utilisé à la fois à la
...
// Chemin repo <-> clé de gabarit (voir app/ssg/default-templates.js pour les clés :
...
// sauvegarde depuis l'écran "Mise en page" (openLayoutEditor() dans app/app.js).
...
// Libellés affichés sur l'écran "Mise en page" (app/app.js) — mêmes clés que
...
// /a-propos/) — même convention que pageUrl() dans app/ssg/content-loader.js (dupliquée
...
// app/ssg/default-templates.js, se charge de l'affichage). Le titre est le seul réglage
...
// donc pas encore sur main — voir openEditor() dans app/app.js) : un unique bloc RichText
...
// (voir app/site/repo-cache.js ; getRepoFiles ne retélécharge que si le sha HEAD distant a changé
...
// URL absolue du site publié (voir app/ssg/context.js) : le domaine personnalisé
...
// (app/site/repo-cache.js), le contenu du dépôt de site tel quel sur la branche main.
...
// changement de renderer — voir le commentaire sur ForgejoApi.publishFiles, app/providers/api.js).
...
// BLOG_SECTION dans app/ssg/renderer.jsx), dupliquée ici plutôt qu'importée (ce fichier
// reste un script classique, pas un module).
const LAYOUT_PREVIEW_BLOG_SECTION = { title: "Blog", slug: "blog", url: "/blog/" };

// Données nécessaires à l'écran "Mise en page" (openLayoutEditor() dans app/app.js) pour un
...
// de Puck, voir openLayoutEditor() dans app/app.js) et republie immédiatement le site avec —
