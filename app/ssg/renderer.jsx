/**
 * Orchestrateur qui parcourt les routes du site, résout le gabarit Puck + le contenu de
 * chaque page via buildContext() (app/ssg/context.js), rend chaque page avec <Render> de
 * Puck + renderToStaticMarkup, et écrit le résultat dans une map { chemin: contenu }.
 *
 * Ne pré-résout PAS les props du gabarit Puck avant <Render> : seul le Context racine
 * est fourni, via <SsgContext.Provider> (voir app/puck/ssg-context.js). Chaque composant
 ...
 *
 * Les gabarits sont fournis par l'appelant (`templates`), un par type de route — voir
 * LAYOUT_TEMPLATE_FILES dans app/site/site-builder.js pour leur stockage dans le dépôt.
 */
...
import { SsgContext } from "../puck/ssg-context.js";
import { puckConfig } from "../puck/registry.jsx";
import { buildRssFeed } from "./feeds/rss.js";
import { buildSitemap } from "./feeds/sitemap.js";
import { mergeItemContentIntoTemplate } from "../puck/template-merge.js";
 ...
 * blog, chaque article, plus rss.xml/sitemap.xml.
 *
 * @param {Object} opts
...
export async function buildSite({ files, title, baseUrl, templates }) {
  ...
  // Encodé en Uint8Array pour tout le monde, jamais en string : le consommateur
  // (api.publishFiles(), qui base64-encode via bytesToBase64()) attend des octets, pas
  // du texte.
