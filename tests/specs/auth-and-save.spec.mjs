...
test.beforeEach(async ({ page }) => {
  // Fait pointer app/auth/config.js vers l'instance Forgejo locale plutôt que Codeberg (voir CONFIG dans app/auth/config.js).
...
test("login OAuth2+PKCE réel, édition et commit d'un fichier Markdown", async ({ page }) => {
  ...
  // avec la vraie nav générée par le composant Puck Nav (app/puck/components/nav.jsx) —
