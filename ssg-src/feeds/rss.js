/**
 * Track E (voir docs/plan-puck-ssg.md) : générateur de flux RSS 2.0 en JS pur, en
 * remplacement de `generate_feed = true` côté Zola (qui écrivait `rss.xml`, voir
 * `feed_filename = "rss.xml"` dans buildConfigToml() de site-builder.js — même nom de
 * fichier gardé côté appelant pour ne pas casser les liens déjà publiés/indexés).
 *
 * N'a besoin que de context.site (title, baseUrl) et de la collection à publier — voir
 * ssg-src/types.js pour la forme de Context/Collections/ContentItem.
 */
...
// api.js/site-builder.js) et url (ContentItem.url) commence toujours par "/" (voir
 ...
 * de date, voir ssg-src/context.js).
 *
